...
public class PlayerInventoryMenu extends PaginatedMenu {
	
    public PlayerInventoryMenu(PlayerMU pmu) {
        super(pmu);
    }
    ...
    public String getMenuName() {
        ...
        return Lang.PLAYER_INVENTORY_MENU_TITLE.getString(playerName);
    }
	...
	public void setMenuItems() {
        ...
        if (off.isOnline()) {
        	if(Bukkit.getEntity(uuid) instanceof Player){
        		...
        		inventory.setItem(39, inv.getItemInOffHand());
        		inventory.setItem(41, inv.getHelmet());
        		inventory.setItem(42, inv.getChestplate());
        		inventory.setItem(43, inv.getLeggings());
        		inventory.setItem(44, inv.getBoots());
        		ItemStack[] is = inv.getStorageContents();
        		for (int i = 0; i < 9; i++) {
        			if (is[i] != null) {
        				inventory.setItem(i+27, is[i]);
        			} else {
        				inventory.setItem(i+27, null);
        			}
        		}
        		for (int i = 9; i < is.length; i++) {
        			if (is[i] != null) {
        				inventory.setItem(i-9, is[i]);
        			} else {
